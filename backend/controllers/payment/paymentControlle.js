...
class paymentController {
  create_stripe_connect_account = async (req, res) => {
    ...
    try {
        ...
        const accountLink = await stripe.accountLinks.create({
          ...
          refresh_url:
            process.env.mode === "dev"
              ...
              : `${process.env.client_admin_production_url}/refresh`,
          return_url:
            process.env.mode === "dev"
              ? `http://localhost:3000/success?activeCode=${uid}`
              : `${process.env.client_admin_production_url}/success?activeCode=${uid}`,
      ...
      console.log("stripe connect account cereate error" + error.message);
    }
  };
  ...
  active_stripe_connect_account = async (req, res) => {
    ...
    try {
      ...
      if (userStripeInfo) {
        ...
        responseReturn(res, 201, { message: "payment active" });
      } else {
  ...
  send_withdrowl_request = async (req, res) => {
    ...
    try {
      ...
      responseReturn(res, 200, {
        withdrowal,
        message: "withdrowl request send",
      });
    } catch (error) {
      responseReturn(res, 500, { message: "internal server error" });
    }
  ...
  payment_request_confirm = async (req, res) => {
    ...
    try {
      ...
      responseReturn(res, 200, { payment, message: "payment confirm success" });
    } catch (error) {
      console.log(error.message);
      responseReturn(res, 500, { message: "internal server error" });
    }
